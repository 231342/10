...
const db1 = mysql.createConnection({
    ...
    database: 'myapp' // 첫 번째 데이터베이스 이름
});

// 두 번째 데이터베이스 연결 (board)
const db2 = mysql.createConnection({
    host: 'localhost',
    user: 'root', // 실제 사용자 이름
    password: '1234', // 실제 비밀번호
    database: 'board' // 두 번째 데이터베이스 이름
});
...
app.post('/register/user', async (req, res) => {
    ...
    try {
        ...
        const [result] = await db1.promise().execute(
...
app.get('/search', async (req, res) => {
    ...
    try {
        const [results] = await db2.promise().execute(query, [`%${searchTerm}%`]);
        res.json(results);
    } catch (err) {
        console.error(err);
